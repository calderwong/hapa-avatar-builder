...
Queue responses and mint-plan responses include `renderExecutor`. The UI enables **Render next edition** only while the Builder-managed local renderer reports `available: true`. `POST /api/song-card-remints/executor-heartbeat` remains a diagnostic presence signal for older adapters, but an external heartbeat never grants claim authority or makes the renderer available; it reports `protocolCertified: false`, `claimAuthority: false`, and `status: retired`. This prevents a connected legacy worker from bypassing the exact-plan certificate used by the local renderer.

Queue operations are local-admin only: `GET /api/song-card-remints`, `POST /api/song-card-remints/:candidateId/approve`, `POST /api/song-card-remints/:candidateId/cancel`, `POST /api/song-card-remints/enqueue`, `POST /api/song-card-remints/:candidateId/render-local`, and `POST /api/song-card-remints/:candidateId/bind-render-plan`. Planning a Song Card upserts one deduplicated candidate; an explicit successful mint reconciles only its exact bound plan and marks that candidate `minted`. Neither approval, enqueue, restart, local job completion, nor plan binding has mint authority.

The legacy external-worker mutation endpoints are deliberately fail-closed: authenticated requests to `POST /api/song-card-remints/claim` and `POST /api/song-card-remints/:candidateId/jobs/:jobId/result` return `409 external_render_worker_protocol_not_certified`, do not touch the durable queue, and direct the caller to `POST /api/song-card-remints/:candidateId/render-local`. The internal queue claim/result methods remain implementation details of the certificate-aware local bridge; they are not a remote execution protocol.
