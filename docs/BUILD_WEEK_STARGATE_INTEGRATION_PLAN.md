...
**Implementation checkpoint (`NAV-002`, 2026-07-18):** implemented in the canonical Tarot Draw by adapting the signed invitation/hello/ack protocol from `hapa-tarot-stargate-reference@9e59305`. One exact pinned Context Card is copied into a second isolated profile; a live issuer creates a fresh signed expiring Pass from the matching memory-only capability; both profiles independently verify Card ID/revision, Formation, Gate commitment, context commitment, signatures, expiry, and explicit consent over Hyperswarm, Noise SecretStream, and Protomux. The Pass, topic, full address, private keys, and profile paths are excluded from durable output. The product projection keeps the four-Card Gate identity unchanged while cyan Aurora and violet Beacon signature rails converge, chevrons lock, and two labeled peer-presence sigils materialize in the existing event horizon. The bounded proof presses a separate safe Result Card. The full truth and evidence audit is `docs/audits/2026-07-18-stargate-gate-pass-arrival.md`.
...
**STG-012 Context Forge checkpoint (`2026-07-19`):** the active Gate now freezes one to eight human-selected Card revisions into an append-only ordered Context Packet, with exact source, selection, Formation, Gate, actor, and time digests. The 3D experience sends Card glyphs over luminous rails into a sealed prism, then distinguishes a mint-green deterministic scaffold from a magenta local-provider proposal. Deterministic mode never claims generation. Ollama mode rejects non-loopback or credential-bearing endpoints and records concrete provider, adapter, model, system/user prompt, response, usage, output, and run provenance. A separate self-illustrated Result Card rises from the Gate as `proposed_unminted`; sources remain unchanged and only a later human review may mint. The isolated product capture contains one real Ollama 0.24.0 / `qwen3.5:27b` invocation at its reported model digest, not a provider fixture. See `docs/STARGATE_CONTEXT_FORGE.md` and `artifacts/demos/STG-012/`.

**STG-013 Wisdom Council checkpoint (`2026-07-19`):** one to three pinned Hapa Wisdom Studio Cards now evaluate the same frozen Context Packet in peer-blind provider-model seats. Three cyan/gold/magenta sentinel chambers send no sibling prompts or outputs across their partitions; all selected seats seal together or no semantic Council result is appended. A deterministic reducer preserves `scope`, `goal`, `evidence`, `mechanism`, and `true-tradeoff` disagreements without averaging scores or choosing a preferred action. True tradeoffs illuminate a gold human-authority dais. Separate Council Lesson and Result Cards emerge from the existing Stargate as `proposed_unminted`. The isolated capture uses three actual local Ollama 0.24.0 / `qwen3.5:27b` calls and compresses only the idle wait in the video. See `docs/STARGATE_WISDOM_COUNCIL.md` and `artifacts/demos/STG-013/`.
