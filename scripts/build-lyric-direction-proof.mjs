...
for (const file of fs.readdirSync(root).filter((name) => name.endsWith("-video-project.json")).sort()) {
  ...
  const artifacts = buildDirectorV2Artifacts({ project: payload, manifest, registry, duration: Math.min(60, Number(project.duration || 60)), recipe: "visualizer-forward", seed: `lyric-direction:${project.song_id}` });
