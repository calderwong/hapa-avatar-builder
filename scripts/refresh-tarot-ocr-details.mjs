...
function findExplicitTitle(lines = [], text = "", card = {}) {
  ...
  if (!isGenericTitle(card.tarotCard?.title || card.title)) return titleCase(card.tarotCard?.title || card.title);
  return titleCase(card.shipCard?.title || card.title || "Tarot Card");
}
...
function isGenericTitle(title = "") {
  return /^(hapa tarot system|protocol|major arcana|minor arcana|mimi tarot|tarot card|card|unknown|chatgpt image|grok)$/i.test(normalizeSpaces(title));
}
