...
import { BadgeCheck, BookOpenCheck, CircleDot, Grid3X3, Link2, ListChecks, Maximize2, Minimize2, Pause, Play, RefreshCw, Route, Shuffle, Sparkles, UserRound, Volume2, Waves, X } from "lucide-react";
...
export default function TarotDraw3DView({ cards = [], avatarName = "Hapa", soundEnabled = false, productionAudit = null, onSelectAvatarProfile }) {
  ...
  }, [cards, avatarName, handleReadingClear, handleReadingRequest]);
      ...
      <div className="tarot-draw-controls hapa-panel" data-variant="notch">
        <div className="tarot-control-actions" aria-label="Deck actions">
          ...
          <button
            ...
            title={hud.lyricsEnabled ? "Hide the lyric crawl" : "Show the lyric crawl"}
          >
            <ListChecks size={14} />
        ...
        <div className="tarot-control-grid">
          ...
          <div className="tarot-control-group tarot-card-back-controls" aria-label="Card type backs">
            {cardBackPiles.map((pile) => (
              ...
              >
                <span className="tarot-card-back-thumb">
                  <img src={pile.imageUri} alt="" loading="lazy" />
                </span>
...
function createTarotDrawGame({ canvas, cards, avatarName, soundEnabled = false, onHud, onReadingRequest, onReadingClear }) {
  ...
  function drawFromPile(pileId) {
    ...
    if (requestedPileIds.has("song") || isSongCard(card)) {
      const { entry, origin } = createInstantDealEntry(card, pileIndex);
      lockEntryInDropZone(entry);
      animateInstantDealEntry(entry, origin, pileIndex, 0.9);
      selectEntry(entry);
  ...
  function autoDealInstantStart() {
    ...
    const mediaCards = [];
    try {
      while (mediaCards.length < 6 && deckCards.length) {
  ...
  function createInstantDealEntry(card, index = 0) {
    const entry = createCardEntry(card);
    const origin = DECK_POSITION.clone().add(new THREE.Vector3(
      ((index % 4) - 1.5) * 0.035,
      index * 0.012,
      (index % 2) * 0.025
    ));
  ...
  function clear() {
    ...
    disposeSpawnNetwork();
    stopDropZonePreview();
    const preservedDock = new Set(dockEntries.filter(Boolean));
  ...
  function clearDeal({ preserveLocked = true } = {}) {
    disposeSpawnNetwork();
    const preservedEntries = preserveLocked ? new Set(lockedZoneEntries()) : new Set();
  ...
  function lockedZoneEntries() {
    ...
    for (const entry of placedEntries) {
      if (entry && (entry.lockedDropZone || entry.lockedMediaPool || entry.lockedCenterVisualizer || entry.lockedDock) && !entries.includes(entry)) {
        entries.push(entry);
      }
    }
  ...
  function currentCenterPreviewSources() {
    ...
    return uniqueDropZoneVideoSources(orderedCenterEntries.flatMap((entry, stackIndex) =>
  ...
  function rebuildDropZonePreviewFromBoardNow() {
    ...
    stopDropZonePreview({ stopSong: true });
    if (!sources.length && !centerSources.length) return;
    const song = dropZoneEntry ? pickDropZoneSong(dropZoneEntry.card) : null;
  ...
  function pickUp(entry) {
    ...
    status = `Lifted: ${entry.card.title}`;
    canvas.style.cursor = "grabbing";
    updateVideoPlayback(entry);
    publishHud(true);
    requestReading("pickup");
  }
  ...
  function centerVisualizerNeedsActiveLoop() {
    return centerVisualizerEnabled && (
  ...
  function animate() {
    ...
    const lyricActive = lyricsEnabled && playing && Boolean(dropZoneEntry);
  ...
  function updateEntry(entry, delta, elapsed) {
    ...
    const lockedZone = entry.lockedDropZone || entry.lockedMediaPool || entry.lockedCenterVisualizer || dockLocked;
  ...
  function updateVideoEdgeMattes(elapsed) {
    ...
    updateVideoEdgeMatte(dockBackgroundPlayer.screen?.matte, elapsed, videoBackgroundKeying && Boolean(dockBackgroundPlayer.sources?.length) && !videoSourceHasAlpha(dockBackgroundPlayer.screen?.source));
    const screens = dropPreview?.screens || [];
  ...
  function detachEntryCardFaceVideo(entry) {
    ...
    if (entry.faceMaterial) {
      if (entry.posterTexture) {
        entry.faceMaterial.map = entry.posterTexture;
        entry.faceMaterial.emissiveMap = entry.videoSourceUri ? entry.posterTexture : null;
      }
      entry.faceMaterial.emissiveIntensity = entry.videoSourceUri ? 0.1 : 0.16;
      const keyState = entry.faceMaterial.userData?.hapaVideoBackgroundKey;
      if (keyState) {
  ...
  function requestReading(reason) {
    const readingEntries = placedEntries.filter((entry) => !entry.lockedDock);
  ...
  function updateCameraRail(delta) {
    cameraRailBlend = smoothBlend(CAMERA_RAIL_HORIZON_START, CAMERA_RAIL_HORIZON_END, cameraPolarFromBoard());
    const target = boardCameraTarget(boardCameraTargetScratch).lerp(horizonCameraTarget(horizonCameraTargetScratch), cameraRailBlend);
    controls.target.lerp(target, 1 - Math.pow(0.006, delta));
    const boardFov = compactMode ? 58 : 48;
    const horizonFov = compactMode ? 54 : 45.5;
  ...
  function resize() {
    ...
    if (canvas.width !== Math.floor(width * renderer.getPixelRatio()) || canvas.height !== Math.floor(height * renderer.getPixelRatio())) {
      ...
      if (nextCompact !== compactMode) {
        ...
        controls.maxDistance = compactMode ? 9.6 : 8.8;
  ...
  function dispose() {
    ...
    audio.dispose();
    renderer.dispose();
    if (window.__THREE_GAME_DIAGNOSTICS__?.kind === "hapa-tarot-draw") delete window.__THREE_GAME_DIAGNOSTICS__;
  }
...
function createCardFaceMaterial(card, entry) {
  ...
  const hasMotion = Boolean(card.videoUri);
...
function setDockBackgroundPlayerSources(player, sources = [], { force = false, playing = false, allowBackgroundless = false } = {}) {
  ...
  if (!nextSources.length) {
    ...
    setVideoMaterialSourceAlpha(player.screen.material, false);
    if (player.screen.material) player.screen.material.opacity = 0.1;
    return;
  }
  ...
  if (changed || !currentStillAllowed) {
    const source = nextDropZoneMediaSource(player.pool, 0) || nextSources[0];
...
function updateDockBackgroundPlayerFrame(player, elapsed, shouldPlay = false) {
  ...
  if (player.screen.material) {
    player.screen.material.opacity = active ? 0.68 + blend * 0.24 + pulse * 0.018 : 0.1;
  }
  if (player.backing?.material) {
    player.backing.material.opacity = active ? 0.74 + blend * 0.16 : 0.48;
    player.backing.material.emissiveIntensity = 0.18 + blend * 0.34;
  }
...
function setDropZoneScreenSource(screen, source, { sourceIndex = screen.sourceIndex || 0, playing = false, seekOffset = 0 } = {}) {
  ...
  video.addEventListener("loadedmetadata", () => {
    if (screen.loadToken !== token) return;
    if (video.videoWidth && video.videoHeight) resizeDropZonePreviewScreen(screen.mesh, video.videoWidth / video.videoHeight);
    const duration = Number(video.duration);
    if (duration > 0.8) {
      try {
        video.currentTime = Math.min(Math.max(0, seekOffset), Math.max(0, duration - 0.25));
...
function updateDropZonePreviewOpacity(preview, appear = 1, pulse = 0) {
  (preview?.screens || []).forEach((screen) => {
    ...
    const opacity = (rig?.userData?.previewCurrentOpacity || screenMesh?.userData?.previewOpacity || 0.68) * appear + pulse * 0.04;
  ...
  const panelSpecs = [
    {
      ...
      horizonPosition: [0.06, 3.02, -3.68],
      horizonScale: 1.62,
      horizonOpacity: 0.94,
      rotation: [0, 0.03, 0],
      opacity: 0.82
    },
    {
      ...
      horizonPosition: [-3.98, 2.58, -0.08],
      horizonScale: 1.72,
      horizonOpacity: 0.82,
...
function lyricCrawlTimingPayloadForSong(song = {}) {
  ...
  return song?.lyricTiming ||
    ...
    song?.timedLyrics ||
    song?.lyric_timing ||
    (lyrics && typeof lyrics === "object" ? (lyrics.timing || lyrics.lyricTiming || lyrics.timedLines || lyrics.segments || lyrics.lines) : null) ||
    null;
}
  ...
  return sourceLines
    .map((line, index) => {
      ...
      return {
        ...
        text: String(line.text || line.line || line.lyric || line.caption || "").trim(),
        start,
        end,
