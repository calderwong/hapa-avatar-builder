...
const cardIdFor = (origin, localId) => `hapa-card:v1:${b64(origin)}:${b64(localId)}`;
...
export class AvatarOverwindOrigin {
  ...
  envelope(kind, record, revision, occurredAt) {
    ...
    return { schema:"hapa.card.envelope.v1",card_id:cardId,identity:{global_id:cardId,origin_node:this.originNode,origin_local_id:localId,aliases:[nativeAlias(this.originNode,localId)]},
      card_type:kind === "avatar" ? "avatar_card" : kind === "song" ? "song_card" : kind === "song-card-edition" ? "song_card_edition" : String(record.cardType || record.kind || "item_card"),title:String(record.title || record.primaryName || record.name || localId),
      ...
      truth:{state:"origin-acknowledged",record_owner:this.originNode},visibility:String(record.visibility || "fleet"),revision:{number:revision,event_id:null},
      provenance:{created_by_node:this.originNode,source_schema:kind === "avatar" ? "hapa.avatar-card.v1" : kind === "song" ? String(record.schemaVersion || "hapa.song-card.v2") : kind === "song-card-edition" ? String(record.schemaVersion || "hapa.song-card.edition.v1") : "hapa.item-card.v1",source_ref:`avatar-builder://${kind}/${localId}`},
      timestamps:{origin_created_at:String(record.createdAt || occurredAt),origin_updated_at:occurredAt,acknowledged_at:occurredAt},
  ...
  async upload(fetchImpl=fetch){const events=this.pending();if(!events.length)return{ok:true,sent:0,acknowledged:0};try{const response=await fetchImpl(`${this.overwindUrl}/v1/cards/events/batch`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${this.token}`,"x-hapa-origin-node":this.originNode},body:JSON.stringify({events})});const body=await response.json();const results=new Map((body.results||[]).map((row)=>[row.event_id,row]));let acknowledged=0;this.db.exec("BEGIN IMMEDIATE");for(const event of events){const result=results.get(event.event_id);if(result?.durable&&["accepted","idempotent_duplicate"].includes(result.outcome)){this.db.prepare("UPDATE outbox SET state='acknowledged',receipt_json=?,acknowledged_at=?,last_error=NULL WHERE event_id=?").run(canonical(result),new Date().toISOString(),event.event_id);acknowledged++;}else this.db.prepare("UPDATE outbox SET attempts=attempts+1,last_error=? WHERE event_id=?").run(result?.outcome||`http_${response.status}`,event.event_id);}this.db.exec("COMMIT");return{ok:acknowledged===events.length,sent:events.length,acknowledged};}catch(error){for(const event of events)this.db.prepare("UPDATE outbox SET attempts=attempts+1,last_error=? WHERE event_id=?").run(error?.message||String(error),event.event_id);return{ok:false,sent:events.length,acknowledged:0,error:error?.message||String(error)};}}
