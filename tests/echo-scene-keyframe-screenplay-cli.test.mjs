...
function screenplay(process) {
  ...
  const result = { schemaVersion: "hapa.echo.full-song-visual-screenplay.v1", songId: "song-a", sourceRevision: { songContextHash: fixtureHash, lyricsHash: fixtureHash, timingHash: fixtureHash, seedSetHash: fixtureHash, promptPolicyHash: fixtureHash }, semanticMining: { songThesis: "test", emotionalArc: [], teachingOrQuestion: "test", motifLexicon: [], referencePolicy: { rule: "mechanic", literalDepictionAllowed: false } }, avatarContinuity: { seedAssets: [{ avatarId: "blue", assetId: "seed", contentHash: fixtureHash, retrievalHandle: "/seed.png" }], globalInvariants: ["face"], allowedVariation: ["camera"] }, sequencePlan: [{ id: "sequence", counts, diversityGate: { maxAdjacentDuplicateVisualTuples: 2, requireActionOrStateChange: true, intentionalHoldRequiresReason: true } }], generationPolicy: { promptImportMode: "stage_only", imageActivationRequired: true, providerPolicy: "codex-built-in-gpt-image-only", videoPolicy: "held-until-separately-enabled" }, review: { status: "approved-for-selective-activation" }, authoringProvenance: { method: "direct_llm_analysis", requestedModel: "gpt-5.6-terra", agentTaskName: "cli-test-author", sourcePacketHash: fixtureHash, instructionHash: fixtureHash, startedAt: "2026-07-18T11:00:00.000Z", completedAt: at, promptAuthoringPolicy: "no-deterministic-scene-generation", heuristicGeneratorUsed: false, artifactHash: "pending", attestation: { type: "authoring-provenance-v1", artifactHash: "pending", attestedBy: "cli-test-author", attestedAt: at } }, provenance: { contentHash: fixtureHash, createdAt: at } };
