...
test("HyperFrames project generation and pixel capture are offline-only and locally runnable", () => {
  ...
  assert.match(captureSource, /positiveEffectiveOpacity/);
  assert.match(captureSource, /canvasChangedTransitions/);
  assert.match(captureSource, /functionalOk && report\.acceptance\.shaderCanvasNonBlank/);
